...
shinyServer(function(input, output) {
  ...
  # read data  
  model<-read.csv("/Users/divya/R_Project/Fire_Risk_cmu/fire_risk_nonres")   
  ...
  output$table <- renderDataTable({
    res <- as.data.frame(data())[c(11,1,3,8,10,23,4,110,140)] # only display selected columns
    res
  })
  ...
  output$downloadTable <- downloadHandler(
    filename = "table.csv",
    content = function(file) {
      write.csv(as.data.frame(data())[c(11,1,3,8,10,23,4,110,140)], file)
    }
  )
